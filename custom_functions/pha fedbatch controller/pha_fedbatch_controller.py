...
#  - Stage 3 Adaptive Sensor-Stat (pH-stat, DO-stat) with Volume-Scaled Pulse Dosing & Runaway Safety Clamp
#  - Stage 3 Nitrogen Management: Full Starvation, Maintenance Trickle N, or Constant Ratio
#  - Live Feed Rate Scaling Multiplier & Quick Action Foam Backoff Commands
#  - Persistent Runtime State Preservation (preserves elapsed time & totals across UI setting updates)
...
FUNCTION_INFORMATION = {
    ...
    'message': 'Advanced dual-pump controller for microbial fed-batch fermentation. '
               ...
               'live feed rate trim / foam mitigation, persistent timer state across reconfigurations, '
               'and adaptive sensor-stat feedback with overfeed runaway safety protection.',
    ...
    'custom_options': [
        ...
        # Calculation Mode & Kinetics
        {
            ...
            'required': True,
            'options_select': [
                ('direct_rates', 'Direct Manual Rates (Plan Equation F0 & Ratio)'),
        ...
        },
        {
            ...
            'required': True,
            'constraints_pass': constraints_pass_positive_value,
            'name': 'Feed Rate Scale Multiplier (1.0 = 100%)',
            'phrase': 'Active feed rate scaling multiplier (also adjustable live via Trim buttons)'
        },
        {
            'type': 'new_line'
        },
        ...
        },
        {
            ...
            'phrase': 'Concentration of carbon substrate in Pump A feed bottle (g/L)'
        },
        {
            'id': 'stock_n_conc_g_l',
            'type': 'float',
            ...
            'phrase': 'Concentration of nitrogen substrate in Pump B feed bottle (g/L)'
        },
        {
        ...
        # Manual / Direct Rate Parameters (Fallback / Override)
        {
            ...
            'phrase': 'Used when Mode is Direct Manual Rates (F0_A / F0_B)'
        },
        {
            'id': 'f0_pump_a_ml_h',
            'type': 'float',
            ...
            'phrase': 'Initial Pump A carbon/substrate feed rate F0'
        },
        {
            'id': 'f0_pump_b_ml_h',
            'type': 'float',
            ...
            'phrase': 'Initial Pump B nitrogen/co-feed rate F0'
        },
        {
            'type': 'new_line'
        },
        ...
        },
        {
            ...
            'phrase': 'Stage 2 exponential feed cutoff duration (6.13 h)'
        },
        {
            'type': 'new_line'
        },
        ...
        },
        {
            ...
            'phrase': 'Used when policy is Trickle Nitrogen'
        },
        {
        ...
        },
        {
            ...
            'default_value': 6.52,
            'required': True,
            'name': 'Sensor Trigger Threshold (e.g., pH or DO setpoint)'
        },
        {
        ...
        },
        {
            ...
            'required': True,
            'constraints_pass': constraints_pass_positive_value,
            'name': 'Stage 3 Max Feed A Rate Clamp (mL/h)',
            'phrase': 'Safety ceiling on rolling 1-hour Pump A dosage to prevent overfeeding, toxicity, or foam runaway'
        }
    ]
}
...
class CustomModule(AbstractFunction):
    def __init__(self, function, testing=False):
        ...
        self.stage3_window_pulses = []  # List of (timestamp, volume_ml) in last 3600s
    ...
    def initialize(self):
        ...
        # Restore persistent runtime states from DB
        ...
        start_time_val = self.get_custom_option("stage_start_time")
        if start_time_val is not None and float(start_time_val) > 0:
            self.stage_start_time = float(start_time_val)
        else:
            self.stage_start_time = None

        paused_val = self.get_custom_option("is_paused")
        if paused_val is not None:
            self.is_paused = bool(paused_val)

        trim_val = self.get_custom_option("feed_rate_scale")
        if trim_val is not None:
            self.feed_rate_scale = float(trim_val)
        elif self.feed_rate_scale is None:
            self.feed_rate_scale = 1.0

        self.stage3_pulses = int(self.get_custom_option("stage3_pulses") or 0)
        self.stage3_window_pulses = []
    ...
    def process_stage2(self):
        ...
        # Apply Live Feed Rate Scaling Multiplier
        scale = max(float(self.feed_rate_scale or 1.0), 0.0)
        calc_a *= scale
        calc_b *= scale
    ...
    def process_stage3(self):
        ...
        if triggered:
            ...
            # Safety Hourly Dosage Clamp Check
            self.stage3_window_pulses = [(ts, d) for ts, d in self.stage3_window_pulses if (now - ts) <= 3600.0]
            recent_hourly_vol_ml = sum(d for _, d in self.stage3_window_pulses)
            max_hourly = float(self.stage3_max_rate_clamp_ml_h or 15.0)

            if recent_hourly_vol_ml + effective_dose_ml > max_hourly:
                self.logger.warning(
                    f"Stage 3 safety clamp active! Hourly Pump A dosing ({recent_hourly_vol_ml:.2f} mL + {effective_dose_ml:.2f} mL) "
                    f"exceeds max clamp ({max_hourly:.2f} mL/h). Suppressing pulse to prevent overfeeding."
                )
                return
            ...
            self.stage3_window_pulses.append((now, effective_dose_ml))
    ...
    def cmd_trim_minus_10(self, args_dict):
        scale = float(self.feed_rate_scale or 1.0)
        self.feed_rate_scale = max(0.1, round(scale - 0.10, 2))
        self.set_custom_option("feed_rate_scale", self.feed_rate_scale)
        return f"Feed rate trimmed down to {int(self.feed_rate_scale * 100)}%."

    def cmd_trim_plus_10(self, args_dict):
        scale = float(self.feed_rate_scale or 1.0)
        self.feed_rate_scale = min(2.0, round(scale + 0.10, 2))
        self.set_custom_option("feed_rate_scale", self.feed_rate_scale)
        return f"Feed rate increased to {int(self.feed_rate_scale * 100)}%."

    def cmd_trim_foam_backoff(self, args_dict):
        scale = float(self.feed_rate_scale or 1.0)
        self.feed_rate_scale = max(0.1, round(scale - 0.25, 2))
        self.set_custom_option("feed_rate_scale", self.feed_rate_scale)
        return f"Foam backoff applied! Feed rate reduced to {int(self.feed_rate_scale * 100)}%."
    ...
    def function_status(self):
        stages = {1: "Stage 1 (Hold/Idle)", 2: "Stage 2 (Active Growth)", 3: "Stage 3 (Product / Accumulation Phase)"}
        ...
        scale_pct = int((self.feed_rate_scale or 1.0) * 100)
        scale_badge = f"<span style='color: {'#e67e22' if scale_pct != 100 else '#2ecc71'}; font-weight: bold;'>{scale_pct}%</span>"
        state_badge = f"<span style='color: {'#e74c3c' if self.is_paused else '#2ecc71'}; font-weight: bold;'>{'PAUSED' if self.is_paused else 'RUNNING'}</span>"

        now = time.time()
        recent_hourly = sum(d for ts, d in self.stage3_window_pulses if (now - ts) <= 3600.0)

        html = (
            f"<div style='line-height:1.4; font-family: sans-serif; font-size: 13px;'>"
            f"<b>Stage:</b> <b>{stages.get(self.current_stage, 'Unknown')}</b> | <b>State:</b> {state_badge} | <b>Elapsed:</b> {elapsed} | <b>Feed Trim:</b> {scale_badge}<br>"
            f"<b>Mode:</b> {mode_str} | <b>Target Ratio (A:B):</b> {ratio_disp:.2f}:1 | <b>&mu;:</b> {self.growth_rate_mu:.3f} h<sup>-1</sup><br>"
            f"<b>Volume V(t):</b> {self.current_volume_l:.2f} L | <b>Est. Biomass X(t):</b> {self.current_biomass_g_l:.2f} g/L<br>"
            f"<b>Pump A (Feed A / Carbon):</b> {self.rate_a_ml_h:.2f} mL/h (F0: {f0_a_disp:.2f}) | Total: {self.total_a_ml:.2f} mL ({self.cumulative_c_delivered_g:.2f} g)<br>"
            f"<b>Pump B (Feed B / Nitrogen):</b> {self.rate_b_ml_h:.2f} mL/h (F0: {f0_b_disp:.2f}) | Total: {self.total_b_ml:.2f} mL ({self.cumulative_n_delivered_g:.2f} g)<br>"
            f"<b>N-Target Progress:</b> {n_progress}<br>"
            f"<b>Stage 3 Sensor-Stat:</b> {self.stage3_pulses} pulses | Rolling 1h Dose: {recent_hourly:.2f}/{self.stage3_max_rate_clamp_ml_h or 15.0:.1f} mL | Policy: {self.stage3_pump_b_action}"
            f"</div>"
        )
