...
# Output information
OUTPUT_INFORMATION = {
    'output_name_unique': 'peristaltic_pump_dose_ramp',
    'output_name': "{}: Raspberry Pi GPIO".format(lazy_gettext('Peristaltic Pump Dose Ramp')),
    'output_library': 'RPi.GPIO',
    'measurements_dict': measurements_dict,
    'channels_dict': channels_dict,
    'output_types': ['volume', 'on_off'],

    'message': "This output turns a GPIO pin HIGH and LOW to control power to a generic Peristaltic Pump Dose Ramp. "
               "The Peristaltic Pump Dose Ramp can then be turned on for a duration or, after determining the pump's maximum "
    ...
    'custom_channel_options': [
        ...
        {
            'id': 'initial_flow_rate',
            'type': 'float',
            'default_value': 0.0,
            'constraints_pass': constraints_pass_positive_value,
            'name': "{} ({})".format(lazy_gettext('Initial Flow Rate'), lazy_gettext('ml/min')),
            'phrase': 'The initial flow rate in ml/minute'
        },
        {
            'id': 'final_flow_rate',
            'type': 'float',
            'default_value': 0.0,
            'constraints_pass': constraints_pass_positive_value,
            'name': "{} ({})".format(lazy_gettext('Final Flow Rate'), lazy_gettext('ml/min')),
            'phrase': 'The final flow rate in ml/minute'
        },
        {
            # 'id': 'flow_rate',
            # 'type': 'float',
...
class OutputModule(AbstractOutput):
    ...
    def dispense_linearly_increasing_rate(self, amount, initial_rate, final_rate, duration):
        total_dispense_seconds = duration * 60
        self.logger.debug("Total duration to run: {0:.1f} seconds".format(total_dispense_seconds))

        timestamp_start = datetime.datetime.utcnow()
        start_time = time.time()
        end_time = start_time + total_dispense_seconds

        self.currently_dispensing = True

        while time.time() < end_time and self.currently_dispensing:
            elapsed_time = time.time() - start_time
            current_rate = initial_rate + (elapsed_time / total_dispense_seconds) * (final_rate - initial_rate)

            duty_cycle = current_rate / self.options_channels['fastest_dispense_rate_ml_min'][0]
            ...
            self.logger.debug("Output turned on at rate: {0:.2f} ml/min".format(current_rate))
            self.GPIO.output(self.options_channels['pin'][0], self.options_channels['on_state'][0])
        ...
        self.currently_dispensing = False
        self.record_dispersal(amount, total_dispense_seconds, timestamp=timestamp_start)
